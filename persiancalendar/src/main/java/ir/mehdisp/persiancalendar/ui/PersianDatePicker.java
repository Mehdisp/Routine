...
import ir.mehdisp.persiancalendar.util.PersianCalendar;
import ir.mehdisp.persiancalendar.util.PersianCalendarUtils;
import ir.mehdisp.persiancalendar.util.PersianHelper;
...
public class PersianDatePicker extends LinearLayout {
    ...
    private void updateVariablesFromXml(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PersianDatePicker_typeface)) {
            int fontId = a.getResourceId(R.styleable.PersianDatePicker_typeface, -1);
            setTypeFace(ResourcesCompat.getFont(context, fontId));
        }
        if (a.hasValue(R.styleable.PersianDatePicker_selected_typeface)) {
            int fontId = a.getResourceId(R.styleable.PersianDatePicker_selected_typeface, -1);
            setSelectedTypeFace(ResourcesCompat.getFont(context, fontId));
        }

        int dividerColor = a.getColor(R.styleable.PersianDatePicker_divider_color, Color.BLACK);
        setDividerColor(dividerColor);

        int textColor = a.getColor(R.styleable.PersianDatePicker_text_color, Color.BLACK);
        setTextColor(textColor);

        int selectedTextColor = a.getColor(R.styleable.PersianDatePicker_selected_text_color, Color.BLACK);
    ...
    public PersianDatePicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        View view = inflate(context, R.layout.sl_persian_date_picker, this);
    ...
    private void updateViewData() {
        ...
        if (displayMonthNames) {
            monthNumberPicker.setDisplayedValues(Constants.INSTANCE.getMONTH_NAMES());
        }
